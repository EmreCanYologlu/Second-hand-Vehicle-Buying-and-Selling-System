...
@listing_routes.route('/listings', methods=['GET', 'POST'])
def view_listings():
   # Ensure the user is logged in
   if 'user_id' not in session:
       flash('You need to log in first!', 'danger')
   ...
   # Initialize search query to be empty
   search_query = ""
   ...
   try:
       ...
       if search_query:
           query = """
               ...
               WHERE
                   ...
                   AND (VehicleAd.title LIKE %s OR Vehicle.brand LIKE %s OR Vehicle.model LIKE %s)
           """
           # Using wildcards (%) for partial matching
           search_term = f"%{search_query}%"
           cursor.execute(query, (search_term, search_term, search_term))
       else:
           # Default query to fetch all available listings if no search term
           query = """
               SELECT
                   VehicleAd.ad_id,
                   VehicleAd.title,
       ...
       return render_template('listings.html', results=listings, search_query=search_query)
